...
app = FastAPI(title="PrimeStride Client OS", version="0.2.0")
...
@app.get("/health")
def health():
    return {"status": "ok", "service": "PrimeStride Client OS", "version": "0.2.0"}
...
def pipeline(request: Request, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse("pipeline.html", {"request": request, "buckets": buckets, "stages": visible})
...
def company_detail(company_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse("company.html", {
        "request": request, "company": c, "stages": PIPELINE_STAGES,
        "modules": MODULES, "completion": discovery_completion(c.discovery)
    })
...
def save_intake(company_id: int, top_improvements: str = Form(""), primary_priority: str = Form(""), current_tools: str = Form(""), company_size: str = Form(""), owner_repetitive_task: str = Form(""), db: Session = Depends(get_db)):
    ...
    db.add(i)
    for f, v in {
        "top_improvements": top_improvements, "primary_priority": primary_priority,
        "current_tools": current_tools, "company_size": company_size,
...
def discovery_meeting(company_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    if not c:
        return HTMLResponse("Company not found", 404)
    return templates.TemplateResponse("discovery_meeting.html", {"request": request, "company": c, "modules": MODULES})

@app.post("/companies/{company_id}/discovery-meeting")
def complete_discovery_meeting(
    company_id: int,
    current_flow: str = Form(""), biggest_bottleneck: str = Form(""), key_person_dependency: str = Form(""), repeated_questions: str = Form(""),
    quote_process: str = Form(""), production_tracking: str = Form(""), management_metrics: str = Form(""), priority_improvement: str = Form(""),
    existing_systems: str = Form(""), success_definition: str = Form(""), baseline_notes: str = Form(""), customer_words: str = Form(""),
    pain1: str = Form(""), pain2: str = Form(""), pain3: str = Form(""),
    module1: str = Form("Low"), module2: str = Form("Low"), module3: str = Form("Low"), module4: str = Form("Low"), module5: str = Form("Low"), module6: str = Form("Low"),
    next_action: str = Form(""), owner: str = Form(""), due_date: str = Form(""), db: Session = Depends(get_db)
):
    c = db.get(Company, company_id)
    d = db.scalar(select(Discovery).where(Discovery.company_id == company_id)) or Discovery(company_id=company_id)
    db.add(d)
    values = {
        "current_flow": current_flow, "biggest_bottleneck": biggest_bottleneck,
        "key_person_dependency": key_person_dependency, "repeated_questions": repeated_questions,
        "quote_process": quote_process, "production_tracking": production_tracking,
        "management_metrics": management_metrics, "priority_improvement": priority_improvement,
        "existing_systems": existing_systems, "success_definition": success_definition,
        "baseline_notes": baseline_notes, "customer_words": customer_words
    }
    ...
    for idx, text in enumerate([p for p in [pain1, pain2, pain3] if p.strip()], 1):
        db.add(PainPoint(
            company_id=company_id, rank=idx, category=f"Priority {idx}",
            description=text.strip(), severity="High" if idx == 1 else "Medium",
...
def followup(company_id: int, request: Request, db: Session = Depends(get_db)):
    ...
    return templates.TemplateResponse("followup.html", {"request": request, "company": c, "meeting": latest})
...
def update_overview(company_id: int, stage: str = Form(...), owner: str = Form(""), next_action: str = Form(""), due_date: str = Form(""), fit_status: str = Form("Unknown"), db: Session = Depends(get_db)):
    c = db.scalar(company_stmt(company_id)); old = c.stage
...
def save_discovery(company_id: int, current_flow: str = Form(""), biggest_bottleneck: str = Form(""), key_person_dependency: str = Form(""), existing_systems: str = Form(""), success_definition: str = Form(""), baseline_notes: str = Form(""), customer_words: str = Form(""), db: Session = Depends(get_db)):
    ...
    for f, v in {
        "current_flow": current_flow, "biggest_bottleneck": biggest_bottleneck,
        "key_person_dependency": key_person_dependency, "existing_systems": existing_systems,
        "success_definition": success_definition, "baseline_notes": baseline_notes,
...
def toggle_task(task_id: int, db: Session = Depends(get_db)):
    t = db.get(Task, task_id); t.done = not t.done; cid = t.company_id; db.commit()
...
def save_readiness(company_id: int, module_no: int = Form(...), score: float = Form(...), status: str = Form(""), notes: str = Form(""), db: Session = Depends(get_db)):
    ...
    db.add(r)
    r.score = max(0, min(100, score)); r.status = status or None; r.notes = notes or None
