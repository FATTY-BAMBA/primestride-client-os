(() => {
  ...
  async function load(box){
    ...
    try{
      ...
      badge.textContent='FIRST-CLASS LINEAGE READY';
      ...
      box.querySelector('[data-ps-latest-copy]').textContent=latest ? `${niceType(latest.job_type)} · ${latest.engine_version||latest.model||'engine recorded'}` : 'Ready for the next retained source';
      ...
      jobs.innerHTML=(data.jobs||[]).slice(0,12).map(j=>`<div class="ps-job-row"><div class="ps-job-type">${esc(niceType(j.job_type))}</div><div class="ps-job-main"><b>${esc(j.result_summary||j.model||j.engine_version||'Processing attempt')}</b><small>${j.source_id?`Source ${esc(short(j.source_id,18))} · `:''}${j.provider_job_id?`Provider ${esc(short(j.provider_job_id,16))} · `:''}${esc(j.updated_at||j.created_at||'')}</small></div><span class="ps-job-status ${esc(j.status||'')}">${esc(niceType(j.status||'unknown'))}</span></div>`).join('');
    }catch(err){
